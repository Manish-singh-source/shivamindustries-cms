...
------------------------------------------------------------------------------------- */
  var ajaxQuoteForm = function () {
    ...
    $quoteForm.on("submit", function (e) {
      e.preventDefault();
      console.log('Quote form submitted via AJAX');
      ...
      $.ajax({
        ...
        dataType: "json",
        beforeSend: function () {
          $form.find(".btn-order").append(loading);
        ...
        success: function (response) {
          ...
          if (response.status === "success") {
            ...
            result = response.message || "Error submitting quote request.";
            cls = "msg-error";
          }
      ...
      if (productCount === 0) {
        $wishlistInner.append(`
          <div class="tf-wishlist-empty text-center">
            ...
            <a href="index.php" class="tf-btn animate-btn btn-back-shop">Back to Shopping</a>
          </div>
        `);
      } else {
...
document.addEventListener('DOMContentLoaded', function() {
    // Shivam Mobile Menu Toggle
    const shivamMobileBtn = document.getElementById('shivamMobileBtn');
    const shivamNavList = document.getElementById('shivamNavList');

    if (shivamMobileBtn) {
        shivamMobileBtn.addEventListener('click', function() {
            shivamNavList.classList.toggle('shivam-active');

            // Change icon
            const icon = this.querySelector('i');
            if (shivamNavList.classList.contains('shivam-active')) {
    ...
    // Shivam Mobile Dropdown Toggle
    if (window.innerWidth <= 991) {
        const shivamDropdownLinks = document.querySelectorAll('.shivam-has-dropdown > a');

        shivamDropdownLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                e.preventDefault();
                const parent = this.parentElement;

                // Close other dropdowns
                document.querySelectorAll('.shivam-has-dropdown').forEach(item => {
                    if (item !== parent) {
                        item.classList.remove('shivam-active');
                    }
                });

                parent.classList.toggle('shivam-active');
            });
        });

        // Shivam Sub-dropdown toggle for mobile
        const shivamSubDropdownLinks = document.querySelectorAll('.shivam-has-subdropdown > a');

        shivamSubDropdownLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                e.preventDefault();
                const parent = this.parentElement;

                // Close other sub-dropdowns
                document.querySelectorAll('.shivam-has-subdropdown').forEach(item => {
                    if (item !== parent) {
                        item.classList.remove('shivam-active');
                    }
                });
    ...
    document.addEventListener('click', function(event) {
        if (window.innerWidth <= 991) {
            const isClickInside = shivamNavList.contains(event.target) ||
                (shivamMobileBtn && shivamMobileBtn.contains(event.target));

            if (!isClickInside && shivamNavList.classList.contains('shivam-active')) {
                shivamNavList.classList.remove('shivam-active');
                if (shivamMobileBtn) {
                    const icon = shivamMobileBtn.querySelector('i');
                    icon.classList.remove('fa-times');
                    icon.classList.add('fa-bars');
                }
            }
    ...
    window.addEventListener('resize', function() {
        if (window.innerWidth > 991) {
            ...
            document.querySelectorAll('.shivam-has-dropdown, .shivam-has-subdropdown').forEach(item => {
                item.classList.remove('shivam-active');
            });
        }
    });
